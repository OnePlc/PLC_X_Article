    ...
    public function checkdbAction()
    {
        ...
        if(! $oRequest->isPost()) {
            ...
            return new ViewModel([
                ...
                'sVendor' => 'oneplace',
                'sModule' => 'oneplace-article',
            ]);
        } else {
            $sSetupConfig = $oRequest->getPost('plc_module_setup_config');

            $sSetupFile = 'vendor/oneplace/oneplace-article/data/install.sql';
            ...
            if($sSetupConfig != '') {
                $sConfigStruct = 'vendor/oneplace/oneplace-article/data/structure_'.$sSetupConfig.'.sql';
                ...
                $sConfigData = 'vendor/oneplace/oneplace-article/data/data_'.$sSetupConfig.'.sql';
                if(file_exists($sConfigData)) {
                    ...
                    $sCustomLangFile = 'vendor/oneplace/oneplace-article/data/de_DE_'.$sSetupConfig.'.po';
                    if(file_exists($sCustomLangFile)) {
                        echo 'got lang file for config '.$sSetupConfig;
                        copy($sCustomLangFile,'vendor/oneplace/oneplace-article/language/de_DE.po');
                        $sCustomLangFile = 'vendor/oneplace/oneplace-article/data/de_DE_'.$sSetupConfig.'.mo';
                        copy($sCustomLangFile,'vendor/oneplace/oneplace-article/language/de_DE.mo');
                    }
            }

            $oModTbl = new TableGateway('core_module', CoreUpdateController::$oDbAdapter);
            $oModTbl->insert([
                'module_key'=>'oneplace-article',
                'type'=>'module',
                'version'=>\OnePlace\Article\Module::VERSION,
                'label'=>'onePlace Article',
                'vendor'=>'oneplace',
            ]);
            ...
            $this->flashMessenger()->addSuccessMessage('Article DB Update successful');
